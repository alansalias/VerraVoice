...
import { requireGuild, isAdmin } from "../permissions";

function fmtList(items: string[]) {
  return items.length ? items.join(", ") : "None";
}

export const handleStatus: CommandHandler = async ({ interaction, store }) => {
  ...
  for (const c of channelsToCheck) {
    ...
    for (const perm of [
      ...
      if (!perms?.has(perm)) lacks.push(perm.toString());
    }
    if (lacks.length) {
  ...
  const issues: string[] = [];
  if (!state?.config?.settlementsCategoryId) issues.push("Missing settlements category (run `/setup init`).");
  if (missingChannels.length) issues.push(`Missing channels: ${fmtList(missingChannels)}.`);
  if (channelChecks.length) issues.push(`Permission gaps: ${channelChecks.join("; ")}.`);
  if (missingRoles.length) issues.push(`Missing roles: ${fmtList(missingRoles)}.`);
