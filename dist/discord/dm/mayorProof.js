...
function pendingRequestsForUser(store, userId) {
    const pending = [];
    for (const [guildId, gs] of Object.entries(store.get().guilds)) {
        for (const req of Object.values(gs.mayorRequests ?? {})) {
            if (req.requesterUserId !== userId)
                continue;
            if (req.status !== "pending")
                continue;
            if (req.proofUrl && req.proofUrl.trim().length)
                continue;
            const settlement = (gs.settlements?.[req.settlementId] ?? null);
            if (!settlement)
                continue;
...
async function handleMayorProofDmMessage(opts) {
