...
exports.dmMayorWelcome = dmMayorWelcome;
const discord_js_1 = require("discord.js");
function guildChannelLink(guildId, channelId) {
    return `https://discord.com/channels/${guildId}/${channelId}`;
}
async function dmMayorWelcome(opts) {
    const { guild, store, mayorUserId, settlementId } = opts;
    const gs = store.get().guilds[guild.id];
    const settlement = (gs?.settlements?.[settlementId] ?? null);
    if (!gs || !settlement)
        return;
