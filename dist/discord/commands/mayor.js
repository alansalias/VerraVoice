...
const handleMayor = async ({ interaction, store }) => {
    ...
    if (sub === "claim") {
        if (guildState && Object.values(guildState.settlements ?? {}).some((s) => s.mayorUserId === interaction.user.id)) {
    ...
    if (sub === "approve" || sub === "deny") {
        if (!(0, permissions_1.canReviewMayorRequests)(interaction, guildState.config)) {
            await interaction.reply({
