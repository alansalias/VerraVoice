...
exports.handleSettlement = void 0;
...
function mustGetGuildState(store, guildId) {
    const guildState = store.get().guilds[guildId];
    if (!guildState)
        throw new Error("This server is not initialized. Run /setup init first.");
    return guildState;
}
function findSettlementByIdOrName(guildState, input) {
...
async function upsertStatusCard(interaction, settlement) {
...
const handleSettlement = async ({ interaction, store }) => {
    ...
    if (sub === "add") {
        ...
        await interaction.deferReply({ flags: v10_1.MessageFlags.Ephemeral });
        ...
        await store.update(async (state) => {
            ...
            let idBase = (0, strings_1.slugify)(name);
            if (!idBase)
                idBase = (0, ids_1.newId)("settlement");
            let id = idBase;
            let i = 2;
        ...
        const channel = await guild.channels.create({
            ...
            permissionOverwrites: [
                ...
                { id: citizenRole.id, allow: [discord_js_1.PermissionFlagsBits.ViewChannel, discord_js_1.PermissionFlagsBits.SendMessages, discord_js_1.PermissionFlagsBits.ReadMessageHistory] },
                { id: viewRole.id, allow: [discord_js_1.PermissionFlagsBits.ViewChannel, discord_js_1.PermissionFlagsBits.ReadMessageHistory], deny: [discord_js_1.PermissionFlagsBits.SendMessages] },
                { id: mayorRole.id, allow: [discord_js_1.PermissionFlagsBits.ViewChannel, discord_js_1.PermissionFlagsBits.SendMessages, discord_js_1.PermissionFlagsBits.ReadMessageHistory] },
                ...modRoleIds.map((id) => ({
                    id,
        ...
        if (updated)
            await upsertStatusCard(interaction, updated);
        await (0, overview_1.upsertGuildOverview)(guild, store);
        await interaction.editReply(`Created settlement **${settlement.name}**.\n- Channel: <#${channel.id}>\n- Mayor role: <@&${mayorRole.id}>`);
        return;
    }
    if (sub === "list") {
        const guildState = store.get().guilds[guild.id];
        const settlements = Object.values(guildState?.settlements ?? {});
        if (!settlements.length) {
            await interaction.reply({ content: "No settlements yet. Use `/settlement add`.", flags: v10_1.MessageFlags.Ephemeral });
            return;
        }
        const lines = settlements
            .sort((a, b) => a.name.localeCompare(b.name))
            .map((s) => `- ${s.name}: tier ${s.tier} (${(0, tiers_1.tierName)(s.tier)}), mayor ${s.mayorUserId ? `<@${s.mayorUserId}>` : "-"}`);
    ...
    if (sub === "info") {
        ...
        const content = [
            `**${settlement.name}**`,
            `- Tier: ${settlement.tier} (${(0, tiers_1.tierName)(settlement.tier)})`,
            `- Mayor: ${settlement.mayorUserId ? `<@${settlement.mayorUserId}>` : "-"}`,
            settlement.channelId ? `- Channel: <#${settlement.channelId}>` : null,
        ]
            .filter(Boolean)
            .join("\n");
        await interaction.reply({ content, flags: v10_1.MessageFlags.Ephemeral });
        return;
    }
    if (sub === "set-tier") {
        const input = interaction.options.getString("settlement", true);
        const tier = interaction.options.getInteger("tier", true);
        const parsedTier = schema_1.SettlementTierSchema.safeParse(tier);
        if (!parsedTier.success) {
            await interaction.reply({ content: "Tier must be 0..5.", flags: v10_1.MessageFlags.Ephemeral });
            return;
        }
        const guildState = store.get().guilds[guild.id];
        if (!guildState) {
        ...
        await (0, overview_1.upsertGuildOverview)(guild, store);
        await interaction.reply({
            content: `Updated **${settlement.name}** tier: ${oldTier} -> ${parsedTier.data} (${(0, tiers_1.tierName)(parsedTier.data)}).`,
            flags: v10_1.MessageFlags.Ephemeral,
        });
        ...
        if (announcementsChannelId && updated) {
            ...
            if (chan && chan.type === discord_js_1.ChannelType.GuildText) {
                await chan.send(`Settlement **${updated.name}** is now tier **${updated.tier}** (${(0, tiers_1.tierName)(updated.tier)}).`);
            }
        }
        return;
    }
    if (sub === "update") {
        ...
        if (!(0, permissions_1.canManageSettlement)(member, settlement, admin)) {
            await interaction.reply({ content: "Only the settlement mayor (or an admin) can update this.", flags: v10_1.MessageFlags.Ephemeral });
            return;
        }
        ...
        if (updated)
            await upsertStatusCard(interaction, updated);
        await (0, overview_1.upsertGuildOverview)(guild, store);
        await interaction.reply({ content: `Updated **${settlement.name}** status card.`, flags: v10_1.MessageFlags.Ephemeral });
        return;
    }
    if (sub === "announce") {
        const guildState = store.get().guilds[guild.id];
        ...
        await interaction.reply({ content: `Announcement sent in <#${targetChannelId}>.`, flags: v10_1.MessageFlags.Ephemeral });
        return;
    }
    if (sub === "destroyed") {
        const guildState = store.get().guilds[guild.id];
        ...
        const member = interaction.member;
        if (!(0, permissions_1.canManageSettlement)(member, settlement, admin)) {
            await interaction.reply({ content: "Only the settlement mayor (or an admin) can declare this.", flags: v10_1.MessageFlags.Ephemeral });
            return;
        }
        ...
        if (announcementsChannelId) {
            const chan = await guild.channels.fetch(announcementsChannelId).catch(() => null);
            if (chan && chan.type === discord_js_1.ChannelType.GuildText) {
                await chan.send(`Settlement **${settlement.name}** was declared **destroyed** (tier reset to 0).${reason?.trim() ? `\nReason: ${reason.trim()}` : ""}`);
            }
        }
        await interaction.reply({
...
exports.handleSettlement = handleSettlement;
